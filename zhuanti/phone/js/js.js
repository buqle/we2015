...
var myLib = {css3Me: function() {
        var self = this;
        self.css3JobList = [];
        self.$ = function(obj) {
            return new self.css3Animate(obj)
        };
        this.css3Animate = function(obj) {
            var _self = this;
            _self._obj = obj;
            _self.jobList = [];
            _self.classList = [];
            _self._obj.defaultClass = _self._obj.defaultClass || obj.className;
            _self.setClass = function(classNew, killOld) {
                _self._obj.defaultClass = (killOld ? "" : _self._obj.defaultClass) + " " + classNew
            };
            _self.animate = function(classNew, timeout, callback) {
                if (_self.jobList.length > 0) {
                    _self.jobList[_self.jobList.length - 1][3] = function() {
                        _self.replace(classNew)
                    }
                } else {
                    _self.replace(classNew)
                }
                _self.jobList.push([function() {
                        callback && callback()
                    }, new Date, timeout]);
                self.css3JobList.push(function() {
                    _self.test()
                });
                return _self
            };
            _self.replace = function(classNew) {
                _self._obj.className = _self._obj.defaultClass + " " + classNew
            };
            _self.stop = function() {
                _self.test(true);
                return _self
            };
            _self.test = function(timeout) {
                if (_self.jobList.length) {
                    var _job = _self.jobList[0];
                    if (_job[3]) {
                        _job[3]();
                        _job[3] = null
                    }
                    if (timeout || new Date - _job[1] > _job[2]) {
                        _job[0]();
                        _self.jobList.shift()
                    }
                }
        ...
        setInterval(function() {
            for (var i = 0; i < self.css3JobList.length; i++) {
                self.css3JobList[i]()
            }
        }, 15)
    }, killMouseDefault: function(e) {
        e = e || event;
        ...
        var u = null;
        ua.replace(/(android)|(iphone)|(msie\s*\d+)|(Trident.+?rv\:\d+)|(webkit)/i, function() {
            for (var a = arguments, i = 1; i <= 7; i++)
                if (u === null && a[i])
                    u = a[i]
        });
        return(u.match(/Trident/i) ? u.replace(/Trident.+?rv\:(\d+)/gi, "msie $1") : u || "other").toLowerCase()
    }(navigator.userAgent).toLowerCase(), hasMouseOut: function(e, t) {
        e = e || event;
        var elem = $(e.currentTarget), fromElem = $(e.relatedTarget);
        if ($(t).has(fromElem).length || fromElem[0] == t)
            return 0;
        else
            return true
    }, scrollX: function() {
        ...
        return(location + "").match(/\?.+$|$/g)[0].replace("?", "").match(new RegExp("(^|&)" + key + "=(.*?)($|&)|$", "ig"))[0].replace(/^.+?=|&/g, "")
    }, runScroll: {interval: 0, resetTo: function(nt, callback) {
            var _self = this;
            if (_self.interval)
                return"falied";
            _self.ny_ = myLib.scrollY();
            if (_self.ny_ > 0)
                ;
            else
                _self.ny_ = 0;
            _self.interval = setInterval(function() {
                var _step = (nt - _self.ny_) / 5;
                if (_step < -100)
                    _step = -100;
                if (_step > 100)
                    _step = 100;
                var ny = _self.ny_ += _step;
                if (callback == 1 || Math.abs(ny - nt) < 1) {
                    ny = nt;
                ...
                scrollTo(myLib.scrollX(), ny)
            }, 15)
        }}, wheelEvent: function(_scrollUp, _scrollDown) {
        var _self = this;
        var kill = myLib.killMouseDefault;
        var scrollFunc = function(e, t) {
            e = e || event;
            kill(e);
            var code = e.wheelDelta || e.detail;
            if (myLib.runScroll.interval)
                return;
            if (code * (t || 1) > 0) {
                _scrollUp()
            } else {
        ...
        self.rightList = {};
        self._delay = 0;
        self._update = new Date;
        self._doc = document;
        self.setDoc = function(d) {
            self._doc = d;
            return self
        };
        self.canRun = function() {
            var n = new Date;
            if (n - self._update > self._delay) {
                self._update = n;
                return true
            }
            return false
        };
        self.exec = function(list) {
            if (self.canRun()) {
                for (var e in list) {
                    list[e]()
                }
        ...
        self.setDelay = function(_time) {
            self._delay = _time;
            return self
        };
        self.addUp = function(fid, func) {
            self.upList[fid] = func;
            return self
        };
        self.addDown = function(fid, func) {
            self.downList[fid] = func;
            return self
        };
        self.addLeft = function(fid, func) {
            self.leftList[fid] = func;
            return self
        };
        self.addRight = function(fid, func) {
            self.rightList[fid] = func;
            return self
        };
        self.bind = function() {
            var doc = self._doc;
            if (doc.addEventListener) {
                ...
                doc.addEventListener("touchend", function(e) {
                    var t = e.changedTouches[0];
                    var zx = Math.abs(self.tx - t.screenX), zy = Math.abs(self.ty - t.screenY);
                    if (zx + zy < 20)
                        return;
                    if (zx > zy) {
                        if (self.tx < t.screenX) {
                            self.exec(self.rightList)
                        } else {
                            self.exec(self.leftList)
                        }
                    } else {
                        if (self.ty < t.screenY) {
                            self.exec(self.upList)
                        } else {
                            self.exec(self.downList)
                        }
                    }
                }, 0)
            }
    ...
    }, imgCut: function(w, h, nw, nh) {
        var zw = nw / w, zh = nh / h, cx, cy;
        if (zw < zh) {
            nw = nw / zw;
            nh = nh / zw;
            cx = 0;
        ...
        return[nw, nh, cx, cy]
    }, getAnimationClass: function(animation) {
        switch (animation) {
            case"touch-down":
                outClass = "px-page-upFromCenter px-hide px-left0";
                inClass = "px-page-upFromBottom px-show px-left0";
                break;
            case"touch-up":
                outClass = "px-page-downFromCenter px-hide px-left0";
                inClass = "px-page-downFromTop px-show px-left0";
                break;
            case"touch-left":
                outClass = "px-page-leftFromCenter px-hide px-top0";
                inClass = "px-page-leftFromRight px-show px-top0";
                break;
            case"touch-right":
                outClass = "px-page-rightFromCenter px-hide px-top0";
                inClass = "px-page-rightFromLeft px-show px-top0";
                break;
            default:
...
var _css3Animate, _touchEvent, _resetScroll, _wheelEvent, _getAnimationClass, _imgCut, _touchEnd;
with (myLib) {
    _css3Animate = new css3Me;
    _touchEvent = new touchScreenEvent;
    _resetScroll = runScroll.resetTo;
    _wheelEvent = wheelEvent;
    _getAnimationClass = getAnimationClass;
    _imgCut = imgCut;
    _touchEnd = touchEnd;
    if (navUA == "iphone") {
        $(".down-iphone").css("display", "block");
        $(".down-android").css("display", "none")
    }
}
function showPage(page) {
    var _parent = $(page._obj), _t;
    while (_t = _parent.children()) {
        for (var i = 0; i < _t.length; i++) {
            var t = $(_t[i]);
            if (t.attr("animate-class")) {
                (function(t, _c, _t) {
                    setTimeout(function() {
                        t.addClass(_c)
                    }, _t * 1 || 1)
                })(t, t.attr("animate-class"), t.attr("animate-timeout"))
            }
        }
        if (!_parent.length)
            return;
...
function hidePage(page) {
    for (var _t = $(page._obj).children(), i = 0; i < _t.length; i++) {
        var t = $(_t[i]);
        if (t.attr("animate-class") && t.attr("need-kill")) {
            t.removeClass(t.attr("animate-class"))
        }
    }
}
function getPage(target) {
    target = typeof target == "string" ? $(className)[0] : target;
    if (target) {
        return _css3Animate.$(target)
    } else {
        return null
    }
}
function getPages(_new, counter) {
    var ret;
    if (currentPage + _new >= 1 && currentPage + _new < maxPage) {
        currentPage += _new;
        ret = {pageOld: getPage(pageList[currentPage - _new]), pageNew: getPage(pageList[currentPage])};
        if (!counter) {
            hidePage(ret.pageOld);
            showPage(ret.pageNew)
        }
        return ret
    }
    return null
}
var currentPage = 0, maxPage = 8, pageList = $(".container");
function pageDown(counter) {
    var ani = _getAnimationClass("touch-down");
    var page = getPages(1, counter);
    if (page) {
        page.pageOld.stop().animate(ani[0], 500);
        page.pageNew.stop().animate(ani[1], 500)
    }
}
function pageUp() {
    var ani = _getAnimationClass("touch-up");
    var page = getPages(-1);
    if (page) {
        page.pageOld.stop().animate(ani[0], 500);
        page.pageNew.stop().animate(ani[1], 500)
...
_touchEnd($(".download-ico")[0], function() {
    for (var i = 0; i < 7; i++)
        pageDown(true)
});
$(function() {
...
function getConfig() {
    var url = "/ajax/Appversion/index";
    $.ajax({url: url, type: "GET", dataType: "json", success: function(t) {
            if (t.data)
                for (var e in t.data)
                    for (var n in t.data[e])
                        "ios" == e ? "downloadUrl" == n ? $("#i-" + n).length && $("#i-" + n).attr("href", t.data[e][n]) : $("#i-" + n).length && $("#i-" + n).html(t.data[e][n]) : "downloadUrl" == n ? $("#a-" + n).length && $("#a-" + n).attr("href", t.data[e][n]) : $("#a-" + n).length && $("#a-" + n).html(t.data[e][n]);
            else
                alert("鏈嶅姟鍣ㄧ箒蹇�,璇峰埛鏂伴�?闈㈤噸璇曪紒")
        }, error: function() {
        }})
}
getConfig();
function resize() {
    var _width = $(".container").width(), _height = $(".container").height();
    for (var i = 0, list = $(".background-cut"); i < list.length; i++) {
        list[i].dwidth = list[i].dwidth || list[i].width;
        list[i].dheight = list[i].dheight || list[i].height;
        var area = _imgCut(_width, _height, list[i].dwidth, list[i].dheight);
        $(list[i]).css({left: area[2], top: area[3], width: area[0], height: area[1]})
    }
}
$(window).resize(resize);
resize();
_touchEvent.setDoc($("#outsideBox")[0]).bind().setDelay(1100).addUp("up", pageUp).addDown("down", pageDown);